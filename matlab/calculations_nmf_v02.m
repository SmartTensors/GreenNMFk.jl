function [sol,normF,lb,ub, AA,sol_real, normF_real, normF1, sol_all, normF_abs, Qyes] = calculations_nmf_v02(number_of_sources,nd,Nsim,aa,xD,t0,time,S,numT)
	...
	normF = zeros(Nsim,1);% norm F is the OLSQ difference between the Observation and the reconstruction
	normCut = 0;
	Qyes=0;
...
% Defining the function that we will minimize Funf = Sum_i(MixFn(i)-Sum_k(Sources(i,k)))^2
    ...
    disp("SIZES OF MATRIX")
    disp(size(S(i,:)))
    disp(size(zeros(1, (i-1)*numT)))
    return
	...
	cutNum = 5;
 	...
 	while (Real_num < cutNum) & (j_all < 10*Nsim)  % use to be Real_num < 200

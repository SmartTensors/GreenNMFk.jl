...
As    = [0.5; 0.7; 0.3; 0.6]; % Write desired amplitudes in a vector [A1 A2...]
%As = 0.3;
...
Xn = [[-0.9;   -0.8] [-0.1; -0.2] [ -0.2; 0.6] [0.3; 0.55]];
%Xn = [-0.1; -0.2];
Xs = ones(length(As), 3);  
...
%% xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
    ...
    [sol,normF,lb,ub, AA, sol_real,normF_real, normF_abs, Qyes] = calculations_nmf_v02(number_of_sources,nd,Nsim,aa,xD,t0,time,S,numT);
    disp(size(sol));
